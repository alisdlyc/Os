...
void HariMain(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	struct SHEET* key_win1 = open_console(shtctl, memtotal);
	struct SHEET* key_win2 = open_console(shtctl, memtotal);
	...
	sheet_updown(key_win1,   2);
	sheet_updown(key_win2,   3);
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	// 256 * 165
	unsigned char *buf = (unsigned char *) memman_alloc_4k(memman, 2560 * 1650);
	sheet_setbuf(sht, buf, 300, 250, -1); /*无透明色*/
	make_window8(buf, 300, 250, "qwq", 0);
	make_textbox8(sht, 8, 28, 300, 250, COL8_000000);
